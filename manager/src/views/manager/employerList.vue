<template>
  <div>
    <div>
      ...
      <!--雇主列表-->
      <el-table :data="employerList" border style="width: 100%">
        ...
        <el-table-column prop="state" align="center" label="状态">
          <template slot-scope="scope">
            <div v-if="scope.row.state === 0" style="color:#67C23A;">{{'正常'}}</div>
...
<script>
  export default {
    ...
    methods: {
      async init() {
        let res = await this.$api('getAll', this.searchForm);//只是role不一样，查询的过程一样
        this.employerList = res.list;
      },
      search() {
        //携带查询的参数再次查询一下列表
        this.$router.push({
      ...
      showMoreInfo(id) {
        let tmp = this.employerList.find(item => item.id === id);
        ...
        this.moreInfo.address = tmp.prov + '|' + tmp.city + '|' + tmp.address;
